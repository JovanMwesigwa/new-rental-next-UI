...
const PropertyEnviromentImagesUpload: React.FC<Props> = ({
  moveOn,
  routeName,
}) => {
  ...
  const handleSubmit = async () => {
    ...
    imageFiles.forEach(async (image: any) => {
      ...
      try {
        await axios
          .post(
            `${APIURL}/property/add_other_images/${property.property_listing}/`,
            data,
            {
              headers: {
  ...
  console.log("DEBUG: ----------------->", property.property_listing);

  return (
    <div className="w-1/2">
      <div className="py-10">
        <h1 className="text-xl w-2/3 tracking-wide text-black font-semibold">
          Upload photos of the enviroment
        </h1>
...
export default PropertyEnviromentImagesUpload;
