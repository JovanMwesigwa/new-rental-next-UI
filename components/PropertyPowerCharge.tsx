...
const PropertyPowerCharge: React.FC<Props> = ({ moveOn, routeName }) => {
  const [powerCollectionType, setPowerCollectionType] = useState<string | null>(
    null
  );
  ...
  const handleSubmit = async () => {
    const data = {
      power_charge_type: powerCollectionType ? powerCollectionType : "Yaka",
    };
    try {
    ...
    <div className="w-1/2">
      <div className="py-10">
        <h1 className="text-xl w-2/3 tracking-wide text-black font-semibold">
          How do you charge your electricity?
        </h1>
        ...
        <div className="flex flex-row my-10 items-center justify-between">
          <div className="flex flex-1 items-center justify-center">
            <h3 className=" text-lg text-gray-600">
              Power collection charges?
            </h3>
          </div>
          <div className="flex flex-1 flex-row">
            <button
              onClick={() => setPowerCollectionType("Yaka")}
              className={`border-2 relative ${
                powerCollectionType === "Yaka"
                  ? "bg-black text-white"
                  : "border-black"
            ...
            <button
              onClick={() => setPowerCollectionType("Meter")}
              className={`border-2 ${
                powerCollectionType === "Meter"
                  ? "bg-black text-white"
                  : "border-black"
