...
const PropertFrontImagesUpload: React.FC<Props> = ({ moveOn, routeName }) => {
  ...
  const handleSubmit = async () => {
    ...
    imageFiles.forEach(async (image: any) => {
      ...
      data.append("property_listing", property.id);
      data.append("image", image);
      // data.append('type', values.type);

      try {
        await axios
          .post(`${APIURL}/property/add_front_images/${property.id}/`, data, {
            headers: {
    ...
    <div className="w-1/2">
      <div className="py-10">
        <h1 className="text-xl w-2/3 tracking-wide text-black font-semibold">
          Upload photos of your units front
        </h1>
...
export default PropertFrontImagesUpload;
